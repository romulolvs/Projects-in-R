...
churn <- read.csv(paste(dataset_path, "Telco-Customer-Churn.csv", sep = ""))
View(churn)
str(churn)
...
# “0-12 Mês”, “13–24 Mês”, “25–48 Meses”, “49–60 Mês” Mês ”,“> 60 Mês”
...
# Os três principais recursos mais relevantes incluem: Contract, tenure_group e PaperlessBilling.
?anova
anova(LogModel, test = "Chisq")

# Analisando a tabela de variância, podemos ver a queda no desvio ao adicionar cada variável uma de cada vez.
# Adicionar InternetService, Contract e tenure_group reduz significativamente o desvio residual.
# As outras variáveis, como PaymentMethod e Dependents, parecem melhorar menos o modelo, embora todos tenham valores p baixos.
testing$Churn <- as.character(testing$Churn)
testing$Churn[testing$Churn == "No"]  <- "0"
testing$Churn[testing$Churn == "Yes"] <- "1"
fitted.results <- predict(LogModel, newdata = testing, type = 'responde')
...
# 2. Se um cliente em um contrato de um ano ou de dois anos, não importa se ele (ela) tem ou não a PapelessBilling,
#    ele (ela) é menos propenso a se cancelar a assinatura.
# 3. Por outro lado, se um cliente estiver em um contrato mensal, e no grupo de posse de 0 a 12 meses, e usando o PaperlessBilling, 
