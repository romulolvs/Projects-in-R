...
con = dbConnect(RSQLite::SQLite(), dbname=paste(dataset_path, "database.sqlite", sep = ""))
...
write_delim(
  ...
  delim = "\t", path = paste(dataset_path, "player_transactions.txt", sep = ""), col_names = FALSE
)

# Agora importamos as transações registradas no item anterior
playersTrxs <- read_baskets(paste(dataset_path, "player_transactions.txt", sep = ""), sep = "[ \t]+",
...
# Calcula as medidas de centralidade de betweeness usando o igraph, para que possamos ter
# tamanhos diferentes de nós no gráfico de rede
